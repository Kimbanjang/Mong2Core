...
public:
	...
	bool OnGossipHello(Player* pPlayer, Creature* pUnit)
	{
		...
		pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, "모든 형상변화 제거", EQUIPMENT_SLOT_END+2, 0, "모든 형상변화를 제거 하시겟습니까?", 0, false);
		pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "메뉴 갱신", EQUIPMENT_SLOT_END+1, 0);
		...
		case EQUIPMENT_SLOT_END: // Show items you can use
			{
				if (Item* OLD = pPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, uiAction))
				{
						...
						if (Item* pItem = pPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
						{
							...
							if(IsSuitable(pItem, OLD, pPlayer))
								if(Items[GUID].find(Display) == Items[GUID].end())
								{
									...
									pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, pItem->GetTemplate()->Name1, uiAction, Display, "형상변화를 사용하게되면 귀속되어 거래 및 환불이 불가능 합니다.\n계속 진행 하시겟습니까?\n\n"+ pItem->GetTemplate()->Name1, GetSellPrice(OLD), false);
								}
						}
					}
					...
					pPlayer->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, "형상 변화 제거", EQUIPMENT_SLOT_END+3, uiAction, "형상변화 제거 "+(std::string)GetSlotName(uiAction)+"?", 0, false);
					pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "뒤로..", EQUIPMENT_SLOT_END+1, 0);
					pPlayer->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, pUnit->GetGUID());
				}
				else
		...
		case EQUIPMENT_SLOT_END+2: // Remove Transmogrifications
			{
				...
				for (uint8 Slot = EQUIPMENT_SLOT_START; Slot < EQUIPMENT_SLOT_END; Slot++)
					...
					if(removed)
					{
						pPlayer->GetSession()->SendAreaTriggerMessage("착용중인 아이템의 모든 형상변화 제거");
						pPlayer->PlayDirectSound(3337);
					}
					else
						pPlayer->GetSession()->SendNotification("가지고있는 모든 아이템의 형상변화 제거");
					OnGossipHello(pPlayer, pUnit);
			} break;
		case EQUIPMENT_SLOT_END+3: // Remove Transmogrification from single item
			{
				if (Item* pItem = pPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, uiAction))
				{
					if(RemoveDisplay(pItem))
					{
						pPlayer->GetSession()->SendAreaTriggerMessage("[ %s ] 형상변화 제거", GetSlotName(uiAction));
						pPlayer->PlayDirectSound(3337);
					}
					else
						pPlayer->GetSession()->SendNotification("[ %s ]의 해당 형상변화를 제거할수 없습니다.", GetSlotName(uiAction));
				...
				if(Item* OLD = pPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, sender))
				{
					if(Items[GUID].find(uiAction) != Items[GUID].end() && Items[GUID][uiAction]->IsInWorld())
					{
						...
						if(pItem->GetOwnerGUID() == pPlayer->GetGUID() && (pItem->IsInBag() || pItem->GetBagSlot() == INVENTORY_SLOT_BAG_0) && IsSuitable(pItem, OLD, pPlayer))
						{
							...
							pPlayer->PlayDirectSound(3337);
							pPlayer->GetSession()->SendAreaTriggerMessage("[ %s ] 형상변환에 성공하였습니다.!!", GetSlotName(sender));
						}
						else
							pPlayer->GetSession()->SendNotification("해당아이템은 변환 할 수 없습니다.");
					}
					else
						pPlayer->GetSession()->SendNotification("아이템이 선택되지 않았습니다.");
				}
				else
					pPlayer->GetSession()->SendNotification("아이템을 착용해주세요");
				Items[GUID].clear();
