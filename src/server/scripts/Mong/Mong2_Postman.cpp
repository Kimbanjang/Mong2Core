...
class npc_Postman : public CreatureScript
{
	public:
		...
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "도대체 택배가 얼만큼 밀려있는거에요?!", GOSSIP_SENDER_MAIN, _mailCheck);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "내 택배 안왔단 말이에요! 숨 막히니까 빨리 보내주세요!", GOSSIP_SENDER_MAIN, _mailQuickSend);
	    ...
	    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
	    {
			switch(action) // switch(action) @START@
			{
				case _mailCheck:
					...
					// uint32 notSendMailCountV = notSendMailCount; 혹시 쿼리문값을 문자열로 인식시 넣어야함

					if(notSendMailCount < 1)
						player->MonsterWhisper("현재 대기중인 택배가 없습니다.", player->GetGUID());
					else
					{
						char checkMSG[100];
						snprintf(checkMSG, 100, "배송 대기중 택배수 : %u", notSendMailCount);
				...
				case _mailQuickSend:
					...
					if(result)
					{
						...
						MailDraft draft("시간은 금이라구, 친구","포인트샵에서 구입한 물품이 도착 했습니다.");
					...
					player->MonsterWhisper("다음 배송예정 택배를 강제 배송 했습니다.", player->GetGUID());
