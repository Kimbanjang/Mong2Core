...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        {
            case NPC_JAINA:
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "?œì´?˜ë‹˜, ?¤ìŒ ?‘ì „???˜í–‰??ì¤€ë¹„ê? ?˜ì—ˆ?µë‹ˆ??", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
                break;
            case NPC_SYLVANA: 
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "?¤ë°”?˜ìŠ¤?? ?¤ìŒ ?‘ì „???˜í–‰??ì¤€ë¹„ê? ?˜ì—ˆ?µë‹ˆ??", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);    
                break;
        }
    ...
    struct npc_jaina_and_sylvana_HRintroAI : public ScriptedAI
    {
        ...
        void StartEvent()
        {
            if(!m_pInstance)
                return
            sLog->outInfo(LOG_FILTER_WORLDSERVER,"EventMGR: creature %u received signal %u ",me->GetEntry(),m_pInstance->GetData(TYPE_EVENT));
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "ì¤€ë¹„ê? ?ë‚¬?µë‹ˆ??", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
